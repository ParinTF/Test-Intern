...
const { notifyUserUpdate } = require("../socket-manager");

const JWT_SECRET = process.env.JWT_SECRET;
const JWT_EXPIRES_IN = process.env.JWT_EXPIRES_IN || "1h";
exports.register = async (req, res) => {
  try {
    const { username, password, firstName, lastName, cardId, birthDate } =
      req.body;
    ...
    notifyUserUpdate({ action: "register", user: safeUser });
    res
      .status(201)
...
exports.login = async (req, res) => {
  try {
    ...
    notifyUserUpdate({ action: "login", user: safeUser });
...
exports.searchUser = async (req, res) => {
  try {
    ...
    if (username.length > 100) {
      return res
        .status(400)
        .json({ message: "Username query parameter is too long" });
    }
